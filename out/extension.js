...
const fs = __importStar(require("fs")); // [NEW] 동적 CVE DB 로딩용
const path = __importStar(require("path")); // [NEW] 동적 CVE DB 로딩용
/**
...
function vectorizeCodeToTokens(code) {
    ...
    const features = {};
    const add = (k, w = 1) => (features[k] = (features[k] ?? 0) + w);
    // 일반 토큰
    const words = lower.match(/[a-z_][a-z0-9_.]+/g) || [];
    const wordSet = new Set(words);
    // 입력/문자열 결합 힌트
    if (/\b(input|prompt|readline|process\.argv|req\.query|req\.body|request\.getparameter)\b/.test(lower))
        add("userinput", 1);
    if (/("|'|`)\s*\+\s*\w|\w\s*\+\s*("|'|`)/.test(code))
        add("stringConcat", 1);
    // SQL
    ["select", "insert", "update", "delete", "where", "from", "union", "concat"].forEach(k => { if (wordSet.has(k))
        add(k, 1); });
    if (/\bfstring\b|`.*\${.*}`/.test(lower))
        add("fstring", 1);
    if (/\.format\(/.test(lower))
        add("format", 1);
    if (/\bquery\b/.test(lower))
        add("query", 1);
    // 명령 실행/셸
    if (/\b(exec|spawn)\b/.test(lower))
        add("exec", 1), add("spawn", 0.5);
    if (/\b(child_process)\b/.test(lower))
        add("child_process", 1.2);
    if (/\bsystem\(|popen\(|subprocess\.(popen|run|call)\b/.test(lower)) {
        add("system", 1);
        add("popen", 1);
        add("subprocess", 0.5);
    }
    if (/\b(sh|bash)\b/.test(lower))
        add("sh", 0.3), add("bash", 0.3);
    // 파일/환경
    if (/\bfs\.(read|write|unlink|chmod|chown|readdir)/.test(lower))
        add("fs.read", 0.8), add("fs.write", 0.6);
    if (/\bprocess\.env\b|secret|password|credential/.test(lower))
        add("env", 1), add("password", 0.6), add("credential", 0.6);
    // 템플릿/SSTI
    if (/\brender\(|template|ejs|jinja|mustache/.test(lower))
        add("render", 0.8), add("template", 0.6);
    if (/\beval\(|new\s+Function\(/.test(lower))
        add("eval", 1), add("Function", 0.5);
    // 역직렬화
    ...
    // 정규표현식
    if (/(new\s+RegExp|re\.compile|\bre\b)/.test(lower))
        add("regex", 1), add("re.compile", 0.6);
    if (/(a+)+|(\.\*)\1/.test(lower))
        add("catastrophic", 0.8);
    // 네트워크
...
function preciseResourceAndSecurityScan(code) {
    ...
    // 이유 채우기 (증거 표시용)
    if (regexRules.hints.length)
        reasons.push(...regexRules.hints.map(h => `regex:${h}`));
...
/* ---------- 정규식 기반 CVE 룰 ---------- */
function evaluateCveFromCodeRegex(code) {
    const rules = [
